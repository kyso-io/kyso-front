...
const UnpureSidebar = (props: any) => {
  ...
  if (permissions && permissions.organizations) {
    permissions!.organizations.forEach((organization) => {
      organizationSelectorItems.push(
        new OrganizationSelectorItem(organization.display_name, `${router.basePath}/${organization.name}`, (activeOrganization && activeOrganization.sluglified_name) === organization.name),
      );
    });
  }
  ...
  if (permissions && permissions.teams && activeOrganization) {
    mappedTeams = permissions!.teams
      ...
      .map((x) => {
        return new LeftMenuItem(x.name, HashtagIcon, 0, `${router.basePath}/${activeOrganization.sluglified_name}/${Helper.slugify(x.name)}`, Helper.slugify(x.name) === router.query.teamName);
      });
  }
  ...
  if (activeOrganization) {
    breadcrumb.push(new BreadcrumbItem(activeOrganization.display_name, `${router.basePath}/${activeOrganization?.sluglified_name}`, !!(activeOrganization && !activeTeam)));
  }

  if (activeTeam) {
    breadcrumb.push(new BreadcrumbItem(activeTeam.display_name, `${router.basePath}/${activeOrganization?.sluglified_name}/${activeTeam?.sluglified_name}`, !!(activeOrganization && activeTeam)));
  }
  ...
  return (
    <>
      <PureSidebar
        ...
        meta={<Meta title={`Kyso - ${Sanitizer.ifNullReturnDefault(activeOrganization?.display_name, "")}`} description={`${Sanitizer.ifNullReturnDefault(activeOrganization?.display_name, "")}`} />}
      >
