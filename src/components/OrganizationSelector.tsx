...
const OrganizationSelector = (props: IOrganizationSelectorProps) => {
  ...
  return (
    <Menu as="div" className="relative w-full inline-block text-left">
      <div>
        <Menu.Button className="group w-full bg-gray-100 rounded-md px-3.5 py-2 text-sm text-left font-medium text-gray-700 hover:bg-gray-200 hover:outline-none hover:ring-2 hover:ring-offset-gray-100 hover:ring-slate-300">
          <span className="flex w-full justify-between items-center">
            <span className="flex min-w-0 items-center justify-between space-x-3">
              <span className="flex-1 flex flex-col min-w-0">
                <span className="text-gray-900 text-sm font-medium truncate">{currentOrg ? currentOrg.name : `Select ${selectorLabel}`}</span>
          ...
          {currentOrg && (
            <div>
              ...
              <div className="px-4 py-2 text-sm font-light truncate text-indigo-500">
                <a href={`${currentOrg && currentOrg.href}`}>Go to {selectorLabel} dashboard</a>
              </div>
