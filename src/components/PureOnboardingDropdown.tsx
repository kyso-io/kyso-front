...
import { OnboardingProgress } from '@kyso-io/kyso-model';
...
const PureOnboardingDropdown = () => {
  ...
  const [valueContent, setValueContent] = useState(contentPublish);
  ...
  useEffect(() => {
    if (value === 'read') {
      setValueContent(contentRead);
    }
    if (value === 'search') {
      setValueContent(contentSearch);
    }
    if (value === 'profile') {
      setValueContent(contentProfile);
    }
    if (value === 'cli') {
      setValueContent(contentCLI);
    }
